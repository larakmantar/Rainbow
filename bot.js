...
.setDescription(`:rocket: <@!${message.author.id}> adlı kullanıcı tarafından <#${message.channel.id}> kanalına gönderilen mesaj silindi!\n\nSilinen Mesaj: **${message.content}**`)
  .setFooter("Rainbow | Log Sistemi")
...
if (!modlogkanal) return;
  ...
  .setColor("RANDOM")
    ...
    .setDescription(`:rocket: Üye Sunucudan Yasaklandı! \n<@!${message.user.id}>, ${message.user.tag}`)
...
if (!modlogkanal) return;
    if (channel.type === "text") {
                let embed = new Discord.RichEmbed()
                    .setColor('RANDOM')
                .setDescription(`:rocket: ${channel.name} adlı metin kanalı oluşturuldu.`)
                .setFooter(`Rainbow | Log Sistemi Kanal ID: ${channel.id}`)
...
.setTitle("SES KANALI OLUŞTURULDU")
                .setDescription(`:rocket: ${channel.name} adlı ses kanalı oluşturuldu!`)
                .setFooter(`Rainbow | Log Sistemi Kanal ID: ${channel.id}`)
...
if (!modlogkanal) return;
    if (channel.type === "text") {
                let embed = new Discord.RichEmbed()
                    .setColor('RANDOM')
                .setDescription(`:rocket: ${channel.name} adlı metin kanalı silini!`)
                .setFooter(`Rainbow | Log Sistemi Kanal ID: ${channel.id}`)
                modlogkanal.send({embed});
            };
            if (channel.type === "voice") {
...
.setTitle("SES KANALI SİLİNDİ")
                .setDescription(`:rocket: ${channel.name} adlı ses kanalı silindi`)
            .setFooter(`Rainbow | Log Sistemi  Kanal ID: ${channel.id}`)
                modlogkanal.send({embed});
            }
    })
...
message.delete()
message.reply(':rocket: **Hey Dostum Bu Sunucuda Reklam Yasak**').then(msg => msg.delete(7000)) 
}
});
//Frenzy Code
client.on("messageUpdate", async (oldMsg, newMsg) => {
...
newMsg.delete()
oldMsg.reply(':rocket: **Hey Dostum Bu Sunucuda Reklam Yasak**').then(msg => msg.delete(7000)) 
}
});
              ...
              if (!message.member.hasPermission("ADMINISTRATOR")) {
                  ...
                  if (uyarisayisi === null) {
                      let uyari = new Discord.RichEmbed()
                          .setColor("RANDOM")
                          .setFooter('Rainbow', client.user.avatarURL)
                          .setDescription(`:rocket: **<@${message.author.id}> Reklam Kick Sistemine Yakalandın! Reklam Yapmaya Devam Edersen Kickleniceksin (1/3)**`)
                          .setTimestamp()
                  ...
                  if (uyarisayisi === 1) {
                      let uyari = new Discord.RichEmbed()
                          .setColor("RANDOM")
                          .setFooter('Rainbow ', client.user.avatarURL)
                          .setDescription(`:rocket: **<@${message.author.id}> Reklam Kick Sistemine Yakalandın! Reklam Yapmaya Devam Edersen Kickleniceksin (2/3)**`)
                          .setTimestamp()
                      message.channel.send(uyari)
                  }
                  if (uyarisayisi === 2) {
                      ...
                      let uyari = new Discord.RichEmbed()
                          .setColor("RANDOM")
                          .setFooter('Rainbow', client.user.avatarURL)
                          .setDescription(`:rocket: **<@${message.author.id}> 3 Adet Reklam Uyarısı Aldığı İçin Kicklendi. Bir Kez Daha Yaparsa Banlanacak**`)
                          .setTimestamp()
                      message.channel.send(uyari)
                  }
                  if (uyarisayisi === 3) {
                      ...
                      let uyari = new Discord.RichEmbed()
                          .setColor("RANDOM")
                          .setFooter('Rainbow', client.user.avatarURL)
                          .setDescription(`:rocket: **<@${message.author.id}> Kick Yedikten Sonra Tekrar Devam Ettiği İçin Banlandı.**`)
                          .setTimestamp()
                      message.channel.send(uyari)
                  }
...
if (!role) {
  member.addRole(member.guild.roles.get(otorol))
                        ...
                        .setDescription(":rocket: **Sunucuya Yeni Katılan** @" + member.user.tag + " **Kullanıcısına** <@&" + otorol + ">  **Rolü verildi**")
                        .setColor('RANDOM') 
    ...
    member.addRole(member.guild.roles.get(otorol))
                        var embed = new Discord.RichEmbed()
                        .setDescription(`:rocket: **Sunucuya Yeni Katılan** \`${member.user.tag}\` **Kullanıcısına** \`${role}\` **Rolü verildi**`)
                        .setColor('RANDOM') 
                        .setFooter(`Pirate Otorol Sistemi`)
     member.guild.channels.get(i).send(embed) 
...
client.on('message', async (msg, member, guild) => {
  let i = await  db.fetch(`saas_${msg.guild.id}`)
      if(i === 'açık') {
        ...
        msg.reply(':rocket: **Aleyküm Selam Hoşgeldin**');      
      } 
      }
    });
///////////////////////
...
client.on("guildMemberAdd", async member => {
  ...
  if(miran){
    ...
    const amil = new Discord.RichEmbed()
      .setColor("RANDOM")
      .setDescription(`:rocket: **@${member.user.tag}** Adlı Kişiye Tag Verildi!`)
      .setFooter(client.user.username, client.user.avatarURL);
 ...
 console.log(`Oops! ${e}`);
         ...
         client.on("roleDelete", async(role , channel , message , guild) => {
          ...
          role.guild.createRole({name: role.name, color: role.color,  permissions: role.permissions}) 
                role.guild.owner.send(`:rocket: **${role.name}** Adlı Rol Silindi Ve Ben Rolü Tekrar Oluşturdum  :white_check_mark:`)
        ...
        client.on("guildMemberAdd", async member => {
          ...
          member.guild.owner.send(`:rocket: Sunucunuza Bir Bot Eklendi ve Sunucudan Otomatik Olarak Atıldı, Sunucuya Eklenmesini Onaylıyor iseniz \`-giriş-izni ${member.id}\``)
          })
