...
if(!frenzy_sayı || !frenzy_kanal) return message.reply(`**Sayaç Sistemini Ayarlamak İçin Lütfen Sayı ve Kanal Belirtiniz** **Örnek** : \`${prefix}sayaç #kanal 100\``)
if(isNaN(frenzy_sayı)) return message.reply(`**Sayaç Sistemini Ayarlamak İçin Sayıyı Sadece Rakamlardan Yazmalısın**`)
  
await db.set(`TitanCode+SayaçSayı_${message.guild.id}`,frenzy_sayı)  
await db.set(`TitanCode+SayaçKanal_${message.guild.id}`,frenzy_kanal.id)  
